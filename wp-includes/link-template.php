...
function get_the_post_navigation($args = array()) {
    ...
    $previous = get_previous_post_link('<div class="nav-previous"><p>Previous Post:</p>%link</div>', $args['prev_text']);
    $next = get_next_post_link('<div class="nav-next"><p>Next Post:</p>%link</div>', $args['next_text']);

    // Only add markup if there's somewhere to navigate to.
    if ($previous || $next) {
        $navigation = _navigation_markup('<nav class="navigation post-navigation" role="navigation">
        <div class="post-nav-box clear">' . $previous . $next, 'post-navigation' . '    </div><!-- .post-nav-box -->
        </nav><!-- .navigation -->', $args['screen_reader_text']);
    }
...
function get_avatar_data($id_or_email, $args = null) {
    ...
    $url = add_query_arg(
            rawurlencode_deep(array_filter($url_args)), set_url_scheme($url, $args['scheme'])
    );
